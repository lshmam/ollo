  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-12 space-y-24">
      ...
      <section className="space-y-8 text-center">
        <h1 className="text-2xl font-serif">Not all those who wander are lost.</h1>
        <Image
          src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image%201-0VZLNRjamBXTedKkOV0ctPqg7kGBv3.png"
        ...
        <p className="text-xs text-center max-w-sm mx-auto text-gray-600">
          TERRA IS A COMPANION ON UNPLANNED WANDERING providing a link to the invisible guides that compass the way we
          move.
        </p>
      </section>
      ...
      <section className="grid grid-cols-3 gap-8 max-w-sm mx-auto">
        {["mushroom", "flower pattern", "sun", "flower", "splash", "dots"].map((icon, i) => (
      ...
      <section className="flex justify-center gap-8">
        {["bg-purple-400", "bg-gray-400", "bg-green-400"].map((color, i) => (
          <div key={i} className={`w-16 h-16 rounded-full ${color} opacity-80`} />
        ))}
      </section>
      ...
      <footer className="text-center space-y-8">
        <Image
          ...
          className="mx-auto"
        />
        <p className="text-xs text-gray-600 max-w-sm mx-auto">© 2024 Terra. All rights reserved.</p>
      </footer>
    </main>
  )
}
